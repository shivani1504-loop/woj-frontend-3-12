...
const RegStudentDetails = () => {
  ...
  const getStudentData = async () => {
    const response = await axios.get(
      `https://wojindia.com/api/auth/getAllRegStudent`
    );
  ...
  const downloadgetOnlyRegStudent = async () => {
    alert("data");

    window.location.href =
      "https://wojindia.com/api/auth/downloadgetOnlyRegStudent";
  };

  return (
    <>
      <Container>
        <h1 class="text-center">Registered Students Details</h1>
        <div class="container mt-5 mb-5">
          <div style={{ overflowX: "auto" }}>
            <button
              className="btn btn-success mb-1"
              onClick={downloadgetOnlyRegStudent}
            >
              Download List
            </button>
            <table>
...
export default RegStudentDetails;
